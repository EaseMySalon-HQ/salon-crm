...
export type NewPackageFormProps = {
  /** When true, render for settings panel: no full-page shell, optional Cancel */
  embedded?: boolean
  onCancel?: () => void
  /** After successful create; defaults to navigating to /packages when not embedded */
  onSuccess?: () => void
}

export function NewPackageForm({ embedded, onCancel, onSuccess }: NewPackageFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    /** When false, package is single-visit (total_sittings 1, min_service_count 1). */
    sittings_enabled: false,
  })

  const [services, setServices] = useState<SelectedService[]>([])
  /** Manual fixed price vs. price derived from (sum of services − discount). */
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        toast({ title: "Package created" + (res.data.warning ? ` — ${res.data.warning}` : "") })
        if (onSuccess) onSuccess()
        else router.push("/packages")
      } else {
        toast({ title: res.message || "Failed to create", variant: "destructive" })
      }
    } finally {
      setSaving(false)
    }
  ...
  const shellClass = embedded ? "" : "p-6 max-w-5xl mx-auto"

  return (
    <div className={shellClass}>
      {embedded ? (
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5 pb-4 border-b border-slate-200">
          <h4 className="text-base font-semibold text-slate-900">Create Package</h4>
          {onCancel && (
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            <div className={`border rounded-xl p-5 space-y-4 ${embedded ? "bg-white/80" : "bg-white"}`}>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-gray-400">
                  {form.type === "FIXED"
                    ? "Client receives the exact services you specify."
                    : "Client picks services from a pool you define."}
                </p>
              </div>
            </div>

            <div className={`border rounded-xl p-5 space-y-3 ${embedded ? "bg-white/80" : "bg-white"}`}>
            ...
            <div className={`border rounded-xl p-5 space-y-4 ${embedded ? "bg-white/80" : "bg-white"}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={e => set("min_service_count", e.target.value)}
                  />
                </div>
              </div>
              )}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Switch
                      ...
                      onCheckedChange={v => set("never_expires", v)}
                    />
                  </div>
                </div>
                {!form.never_expires && (
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  checked={form.cross_branch_redemption}
                  onCheckedChange={v => set("cross_branch_redemption", v)}
                />
              </div>
            </div>
