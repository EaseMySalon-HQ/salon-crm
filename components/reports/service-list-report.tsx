  ...
  const flattenedRows = useMemo((): ServiceRow[] => {
    ...
    salesData.forEach((sale) => {
      ...
      ;(sale.items || []).forEach((item: any, idx: number) => {
        ...
        const totalDurationMinutes = perUnitDuration * units
        ...
        /** Start–end for one unit slot (sequential blocks when qty > 1) */
        const startEndForUnit = (unitIndex: number) => {
          if (perUnitDuration <= 0 || !saleTimeStr) {
            if (totalDurationMinutes > 0 && saleTimeStr) {
              try {
                ...
                const endStr = formatTimeMins(endMins)
                return `${saleTimeStr} – ${endStr}`
              } catch {
                return saleTimeStr
              }
            }
            return `${saleDateStr} ${saleTimeStr}`.trim() || "—"
          }
          try {
            const [timePart] = saleTimeStr.split(" ")
            const [h, m] = (timePart || "").split(":").map(Number)
            const baseMins = (h || 0) * 60 + (m || 0)
            const s = baseMins + unitIndex * perUnitDuration
            const e = s + perUnitDuration
            return `${formatTimeMins(s)} – ${formatTimeMins(e)}`
          } catch {
            return saleTimeStr
          }
        }
        ...
        const pushRow = (staffName: string, staffId: string | undefined, attributedTotal: number, rowSuffix: string) => {
          ...
          for (let u = 0; u < units; u++) {
            rows.push({
              ...
              totalDurationMinutes: perUnitDuration,
              customer: sale.customerName || "—",
              saleDate,
              saleTime: saleTimeStr,
              startEndTime: startEndForUnit(u) || `${saleDateStr} ${saleTimeStr}`.trim() || "—",
              status: status || "—",
              paidStatus,
